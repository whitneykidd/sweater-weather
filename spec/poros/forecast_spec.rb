  ...
  describe 'class methods' do
    it 'search(juneau, ak)' do
      VCR.use_cassette('forecast juneau') do
        ...
        expect(forecast.current[:sunrise]).to eq(1_596_286_243)
        expect(forecast.current[:sunset]).to eq(1_596_345_425)
        expect(forecast.current[:temperature]).to eq(68)
        expect(forecast.current[:time]).to eq(1_596_321_698)
        expect(forecast.current[:uv_index]).to eq(5.07)
        expect(forecast.current[:visibility]).to eq(10_000)
        expect(forecast.current.length).to eq(9)

        forecast.hourly.each do |hour|
          expect(hour[:temperature]).to be_between(54.95, 68)
          expect(hour[:time]).to be_between(1_596_319_200, 1_596_488_400)
        end
