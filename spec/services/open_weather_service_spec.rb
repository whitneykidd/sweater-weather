...
describe OpenWeatherService do
  it 'can get weather by city and state' do
    VCR.use_cassette('forecast austin') do
      ...
      expect(json[:current][:sunrise]).to eq(1596369028)
      expect(json[:current][:sunset]).to eq(1596417832)
      expect(json[:current][:temp]).to eq(89.29)
      expect(json[:current][:dt]).to eq(1596384462)
      ...
      json[:hourly].each do |hour|
        ...
        expect(hour[:dt]).to be_between(1596384000, 1596996000)                                  
      end
